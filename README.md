        ...
        get_activity_options    get_food_options      ← real OpenStreetMap data
              └───────────┬────────────┘
                  generate_final_plan  (LLM)
                          │
                    estimate_cost  ──→  validate_plan
                          │                  │
                          └──── replan ◄──────┘  (on budget/time violation)
                                   │
...
|---|------|--------------|------|
| 1 | `parse_preferences` | Form **or** free text → structured profile; flags vague input | LLM |
...
| 7 | `estimate_cost` | Sums spend + computes travel from real coordinates | Code |
| 8 | `validate_plan` | Checks budget, time and constraints | Code |
